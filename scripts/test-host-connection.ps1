...
try {
    ...
    Write-Host "[OK] Server is healthy: $($healthResponse.status)" -ForegroundColor Green
} catch {
    Write-Host "[FAIL] Cannot reach server at $ServerUrl" -ForegroundColor Red
...
try {
    ...
    Write-Host "[OK] SignalR negotiate endpoint accessible" -ForegroundColor Green
} catch {
    if ($_.Exception.Response.StatusCode.value__ -eq 401) {
        Write-Host "[OK] SignalR endpoint exists (requires authentication)" -ForegroundColor Green
    } elseif ($_.Exception.Response.StatusCode.value__ -eq 404) {
        Write-Host "[FAIL] SignalR hub not found at /hubs/host" -ForegroundColor Red
        Write-Host "  The HostHub needs to be registered in Program.cs" -ForegroundColor Yellow
    } else {
        Write-Host "[FAIL] SignalR negotiate failed" -ForegroundColor Red
        Write-Host "  Error: $_" -ForegroundColor Red
    }
}
